...
export class PDFService {
  ...
  async generateStudyGuidePDF(data: {
    ...
    // For now, create a simple text-based fallback instead of using Puppeteer
    // This ensures the download works while we resolve system dependencies
    const content = this.generateTextContent(data);
    return Buffer.from(content, 'utf-8');
  }

  private generateTextContent(data: {
    filename: string;
    summary: string;
    questions: any[];
    selectedTypes: string[];
    ...
    let content = `AI-GENERATED STUDY GUIDE\n`;
    content += `${'='.repeat(50)}\n\n`;
    content += `Source Document: ${data.filename}\n`;
    content += `Generated on: ${new Date().toLocaleDateString()}\n`;
    content += `Total Questions: ${data.questions.reduce((sum, section) => sum + section.questions.length, 0)}\n\n`;
    ...
    data.questions.forEach((section) => {
      ...
      content += `${'-'.repeat(typeName.length)}\n\n`;
    ...
    content += `Generated by AI Study Guide Generator\n`;
    content += `Powered by Hugging Face AI Models\n`;
